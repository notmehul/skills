...
Companion to `diagnosing-bugs`, which finds why something is broken. Blast radius runs before it: what a change breaks somewhere else, while it still looks fine.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits for intent (`gh pr view`, `git log -p`).
...
6. For a big or wide change, run it through `interrogate`. Several models get the same question and the answers are merged. Different models catch different real bugs.
