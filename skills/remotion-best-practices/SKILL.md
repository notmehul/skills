...
description: "Build or fix a Remotion video composition in React. Use when writing Remotion components, sequencing a timeline, wiring audio, or rendering output."
metadata:
...
## Before code: the beat sheet

For any video a person will watch (a launch, a demo, an explainer), write a beat table first: time, what's shown, what's said, what's written on screen, motion, audio. Add the 3 to 5 signature moves that make the edit recognisable. Get it approved once, then write compositions. If you're matching a reference video, extract its frames with FFmpeg and read them; a description of an edit is not enough to rebuild it.

- The three channels split the work. Picture, voiceover and on-screen text each carry something the other two don't. Never caption the voiceover or label what the picture already shows.
- Open on the product doing the thing, not a logo or title card. Seconds 3 to 15 continue the hook's premise: if the hook shows the problem, the next beat starts solving it. Logo, team and company story go at the end.
...
**Hook test** (not optional for anything a person will watch): render the first frame and the 3-second frame, `--frame=0` and `--frame=<3 * fps>`, and look at them as a muted viewer would. A stranger must get what this is from those frames alone. If they can't, rework the opening before polishing anything else.
