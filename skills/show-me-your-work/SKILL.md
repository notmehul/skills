...
- Prefer evidence produced by committed scripts over hand-made one-offs, so a reviewer can re-run it. A check you ran by hand is a check the next run has to trust you about.

## Audit the log against the transcript

At the end of the run, before handing back, check the log told the truth. Read this run's own transcript: `~/.claude/projects/<project-slug>/<session-id>.jsonl`, where the slug is the working directory with `/` replaced by `-`. Read only this session's file; globbing across `~/.claude/projects/*/` reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a reviewer on a different model family from the one that did the work: the Agent tool with `subagent_type: codex:codex-rescue`. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. When the codex plugin is unavailable, fall back to `general-purpose` on a different tier (`model: fable` or `sonnet`) and say in the Attention section that the review stayed inside one family. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
